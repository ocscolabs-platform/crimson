...
The CMS is available at the canonical `/crimson-admin-control` path; legacy `/admin` requests redirect there. It uses Supabase Auth sessions, revision-based publishing, and RLS-backed role boundaries. The current release contract and merge gates are documented in [`docs/RELEASE-READINESS.md`](./docs/RELEASE-READINESS.md). The guarded row-copy workflow in [`docs/CMS-PROMOTION.md`](./docs/CMS-PROMOTION.md) is transitional infrastructure only and is not the target editorial workflow.
