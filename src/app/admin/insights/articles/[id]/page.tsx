...
import { getInsightsArticleEditorData, getInsightsTaxonomy } from "@/lib/insights-data";
  ...
  return (
    <main className="admin-page">
      <div className="admin-shell admin-editor-shell">
        ...
        {isDraft ? <div className="admin-editor-panel"><InsightsComposer taxonomy={taxonomy} article={{ ...article, status: "draft" }} /></div> : <section className="admin-editor-panel insights-readonly-article" aria-labelledby="readonly-article-title"><div className="insights-readonly-banner"><strong>{statusLabel(article.status)} is read-only.</strong><span>Review and Published revisions are not silently converted into editable forms.</span></div><dl className="insights-readonly-summary"><div><dt>Slug</dt><dd>{article.slug}</dd></div><div><dt>Excerpt</dt><dd>{article.excerpt || "No excerpt."}</dd></div><div><dt>Category</dt><dd>{article.categoryName}</dd></div><div><dt>Tags</dt><dd>{article.tagNames.length ? article.tagNames.join(", ") : "No tags."}</dd></div></dl><article className="insights-readonly-content"><h2 id="readonly-article-title">{article.title || "Untitled article"}</h2><ReadOnlyArticleBody body={article.body} /></article></section>}
