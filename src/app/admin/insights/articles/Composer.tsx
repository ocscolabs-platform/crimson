  ...
  return (
    <div className="insights-composer-layout">
      <form className="insights-composer" onSubmit={handleSave}>
        ...
        <aside className="insights-metadata-panel" aria-label="Article metadata"><div className="insights-panel-heading"><div><p className="admin-kicker admin-kicker-green">Draft metadata</p><h2>Make it findable.</h2></div><span>Optional until review</span></div><label className="insights-field"><span>Primary Category</span><select name="category_id" value={categoryId} onChange={(event) => { setCategoryId(event.target.value); setCategoryFeedback(initialCategoryState); markDirty(); }}><option value="">No category yet</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>{role === "owner" ? <div className="insights-category-create"><label className="insights-field"><span>Create Category</span><input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} maxLength={80} placeholder="e.g. Practice" /></label><div className="insights-category-actions" aria-label="Category actions"><button className="button insights-category-create-button" type="button" onClick={handleCreateCategory}><span className="insights-category-action-icon" aria-hidden="true">＋</span>Create</button><button className="button insights-category-delete-button" type="button" onClick={handleDeleteCategory} disabled={!categoryId}><svg className="insights-category-action-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M3.5 5.5v7.25c0 .69.56 1.25 1.25 1.25h6.5c.69 0 1.25-.56 1.25-1.25V5.5M6 5.5V4.25C6 3.56 6.56 3 7.25 3h1.5C9.44 3 10 3.56 10 4.25V5.5M2.5 5.5h11M6.5 7.5v4M9.5 7.5v4" /></svg>Delete</button></div>{categoryFeedback.status !== "idle" ? <p className={categoryFeedback.status === "saved" ? "insights-success insights-category-feedback" : "insights-error insights-category-feedback"} role={categoryFeedback.status === "saved" ? "status" : "alert"}>{categoryFeedback.status === "saved" ? "✓ " : ""}{categoryFeedback.message}</p> : null}</div> : null}<fieldset className="insights-tags-field"><legend>Tags <small>Optional</small></legend>{role === "owner" ? <div className="insights-tag-management"><label className="insights-field"><span>Create Tag</span><input value={tagName} onChange={(event) => setTagName(event.target.value)} maxLength={80} placeholder="e.g. Strategy" /></label><div className="insights-category-actions" aria-label="Tag actions"><button className="button insights-category-create-button" type="button" onClick={handleCreateTag}><span className="insights-category-action-icon" aria-hidden="true">＋</span>Create</button><button className="button insights-category-delete-button" type="button" onClick={handleDeleteTag} disabled={!tagIds.length}><svg className="insights-category-action-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M3.5 5.5v7.25c0 .69.56 1.25 1.25 1.25h6.5c.69 0 1.25-.56 1.25-1.25V5.5M6 5.5V4.25C6 3.56 6.56 3 7.25 3h1.5C9.44 3 10 3.56 10 4.25V5.5M2.5 5.5h11M6.5 7.5v4M9.5 7.5v4" /></svg>Delete selected</button></div>{tagFeedback.status !== "idle" ? <p className={tagFeedback.status === "saved" ? "insights-success insights-category-feedback" : "insights-error insights-category-feedback"} role={tagFeedback.status === "saved" ? "status" : "alert"}>{tagFeedback.status === "saved" ? "✓ " : ""}{tagFeedback.message}</p> : null}</div> : null}{tags.length ? <div className="insights-tag-grid">{tags.map((tag) => <label className="insights-check" key={tag.id}><input type="checkbox" name="tag_ids" value={tag.id} checked={tagIds.includes(tag.id)} onChange={() => toggleTag(tag.id)} /><span>{tag.name}</span></label>)}</div> : <p className="insights-muted">{role === "owner" ? "Create a Tag to make this Draft easier to find." : "No approved Tags are available yet."}</p>}</fieldset>{saveFeedback ? <div className="insights-error" role="alert"><strong>{actionMessage(saveFeedback)}</strong>{saveFeedback.issues.length ? <ul>{saveFeedback.issues.map((issue) => <li key={issue}>{issue}</li>)}</ul> : null}<button className="button button-light insights-retry-button" type="button" onClick={() => requestSave("explicit")}>Retry</button></div> : null}<div className="insights-save-row"><div className="insights-save-copy" aria-live="polite"><strong>{saveStatus === "saving" ? "Saving…" : saveStatus === "dirty" ? "Unsaved changes" : saveStatus === "conflict" ? "Conflict — reload required" : saveStatus === "error" ? "Save failed" : hasMounted ? formatSavedAt(lastSavedAt) : article ? "Saved" : "Not saved yet"}</strong><span>{saveStatus === "conflict" ? "Your local changes were not overwritten." : saveStatus === "error" ? "Your local changes are still here." : "Autosaves after a short pause; explicit Save Draft remains available."}</span></div><button className="button button-primary insights-save-button" type="submit" disabled={saveStatus === "saving" && !inFlightRef.current}>{saveStatus === "saving" ? "Saving…" : "Save Draft"}</button></div>{saveStatus === "conflict" ? <button className="button button-light insights-reload-button" type="button" onClick={() => router.refresh()}>Reload latest saved version</button> : null}</aside>
      </form>
