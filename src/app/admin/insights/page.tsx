    ...
    <main className="admin-page">
      <div className="admin-shell admin-editor-shell">
        ...
        <section className="admin-hero insights-hero">
          ...
          <div><p className="admin-intro">A focused space for writing, saving, and preparing Insights Drafts. Review and publishing stay protected behind the existing workflow.</p><Link className="button button-primary insights-new-button" href="/crimson-admin-control/insights/articles/new">New Article</Link></div>
        </section>
        ...
        {membership.role === "owner" ? <section className="insights-review-queue" aria-labelledby="needs-review-heading"><div className="admin-section-heading"><div><p className="admin-kicker admin-kicker-green">Owner attention</p><h2 id="needs-review-heading">Articles requiring your review <span className="insights-count" aria-label={`${data.reviewCount} articles need review`}>{data.reviewCount}</span></h2></div><p className="admin-section-note">All articles remain in the library below; oldest submissions appear first.</p></div>{data.reviewQueue.length ? <div className="insights-review-list">{data.reviewQueue.map((article) => <article className="insights-review-row" key={article.id}><div className="insights-review-row-main"><div><span className="insights-status insights-status-review">Needs Review</span><h3>{article.title}</h3></div><p className="insights-review-row-detail">Submitted {submittedLabel(article.submittedAt)}</p></div><Link className="button button-light" href={`/crimson-admin-control/insights/articles/${article.id}`}>Review ↗</Link></article>)}</div> : <p className="insights-empty-copy">No articles are waiting for review.</p>}</section> : null}
