...
- **Decision:** Keep `feature/* → staging → main` for application code, but move CMS publication to an explicit revision workflow. Draft and review revisions remain private; an owner-only atomic publish operation makes a reviewed revision public. The canonical authenticated CMS entry point is `/crimson-admin-control`; Preview uses the same canonical path, while `/admin` remains a compatibility redirect. The current staging-to-Production row-copy runner is temporary migration infrastructure, not the permanent editorial workflow.
