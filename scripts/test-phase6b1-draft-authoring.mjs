...
test("first-save failures stay actionable and preserve the created identity", async () => {
  ...
  assert.match(composer, /onMouseDown=\{\(event\) => event\.preventDefault\(\)\}/);
...
test("body validator rejects H1, code blocks, unknown nodes, and unsafe links", () => {
  ...
  assert.equal(validateInsightsBody({ ...body, doc: { type: "doc", content: [{ type: "orderedList", attrs: { start: 1, type: null }, content: [{ type: "listItem", content: [{ type: "paragraph", content: [{ type: "text", text: "Safe" }] }] }] }] } }).success, true);
});
